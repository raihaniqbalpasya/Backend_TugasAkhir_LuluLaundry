...
module.exports = {
  async getAll(req, res) {
    try {
      ...
      const data = await faqService.getAll(perPage, start); // Data yang sudah dipaginasi
      const allData = await faqService.getAllData(); // Seluruh data tanpa paginasi
  ...
  async create(req, res) {
    try {
      const data = await faqService.create({
  ...
  async update(req, res) {
    try {
      await faqService.update(req.params.id, req.body);
      const data = await faqService.getById(req.params.id);
      if (data !== null) {
        res.status(200).json({
          status: true,
  ...
  async deleteById(req, res) {
    try {
      const data = await faqService.delete(req.params.id);
      if (data === 1) {
        res.status(200).json({
          status: true,
